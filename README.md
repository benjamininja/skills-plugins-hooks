...
1. **Check** (`python tools/update_vendor_skills.py`) —
   - `skills[]`: pinned vs latest upstream commit on each source path, with `compare` links, **plus local-drift detection**: local files are hash-compared against the pinned upstream tree (git blob SHAs via one trees-API call per unique repo+commit — no downloads), catching silent forks. Drift is *reported, never auto-resolved*, in **two buckets**: **known** (annotated in that entry's `known_local_edits: [{file, reason, local_sha}]` and matching its blessed `local_sha` — quiet one-liner) and **NEW** (loud: unannotated drift, or an annotated file whose content moved past its blessing). For NEW drift: revert, annotate + `--bless`, or promote to `forks[]` per [ADR-0001](docs/adr/0001-vendor-cache-fork-pattern.md). Annotations whose file is no longer drifted are flagged stale. `--no-drift` skips the scan.
   - This is the **three-state ontology** (grilled 2026-07-12): *faithful vendor* (any drift is NEW) / *vendored-with-known-edits* (upstream still source of truth, listed deviations must survive updates) / *fork* (never auto-applied, hand-merge via `vendor-cache/`).
...
2. **Update** (`--apply <name>`) — re-downloads that skill's folder from the latest upstream commit, replaces the local copy, and re-pins `vendor-skills.json`. Warns first which known edits it will overwrite (with reasons) and flags NEW drift loudly; refuses fork names (update the mirror instead, then hand-merge). Afterwards it prints the **reapply ritual**: `git diff HEAD -- <path>` shows your edits as the removed lines (git HEAD *is* the backup — no patch machinery); reapply by hand, then `--bless <name>`.
3. **Bless** (`--bless <name>`) — recomputes `local_sha` for that skill's `known_local_edits` (CRLF-normalized, so hashes survive checkout line-ending rewrites). Run once when annotating, and as the last step of every reapply.
4. Review the resulting `git diff` before committing — upstream skills can change structure or licensing.
